...
class APITestCase(unittest.TestCase):
    ...
    def get_api_headers(self, username, password):
        return {
            #'Authorization': 'Basic ' + b64decode((username + ':' + password).encode('utf-8')).decode('utf-8'),
    ...
    def test_no_auth(self):
        response = self.client.get('/api/v1/posts', content_type='application/json', follow_redirects=True)
        # print('rsp code: ', response.status_code)
        # print('rsp body: ', response.get_data(as_text=True))
    ...
    def test_bad_auth(self):
        ...
        user = User(email=EMAIL_JOHN, password=PASWD_JOHN, confirmed=True, role=role)
        db.session.add(user)
        db.session.commit()

        # authenticate with bad password
        response = self.client.get('/api/v1/posts/', headers=self.get_api_headers(EMAIL_JOHN, '123'))
        self.assertEqual(response.status_code, 401)
    ...
    def test_token_auth(self):
        ...
        # get token
        response = self.client.post('/api/v1/tokens/', headers=self.get_api_headers(EMAIL_JOHN, PASWD_JOHN))
    ...
    def test_unconfirmed_account(self):
        ...
        role = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(role)
        user = User(email=EMAIL_JOHN, password=PASWD_JOHN, role=role)
        db.session.add(user)
        db.session.commit()

        response = self.client.get('/api/v1/posts/', headers=self.get_api_headers(EMAIL_JOHN, PASWD_JOHN))
    ...
    def test_posts(self):
        ...
        # write a empty post
        response = self.client.post('/api/v1/posts/', headers=self.get_api_headers(EMAIL_JOHN, PASWD_JOHN),
        ...
        # get the new post
        response = self.client.get(url, headers=self.get_api_headers(EMAIL_JOHN, PASWD_JOHN))
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        ...
        response = self.client.get('/api/v1/users/{}/posts/'.format(user.id),
                                   headers=self.get_api_headers(EMAIL_JOHN, PASWD_JOHN))
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        ...
        # response = self.client.get('/api/v1/users/{}/timeline/'.format(user.id),
        #                            headers=self.get_api_headers(EMAIL_JOHN, PASWD_JOHN))
        # self.assertEqual(response.status_code, 200)
        # json_response = json.loads(response.get_data(as_text=True))
        # self.assertIsNotNone(json_response.get('posts'))
        ...
        # edit post
        response = self.client.put(url, headers=self.get_api_headers(EMAIL_JOHN, PASWD_JOHN),
